...
export function CompareDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Compare Instances</h2>
        ...
        <div className="overflow-auto flex-1 p-6">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="text-left text-sm font-medium text-gray-500 dark:text-gray-400 pb-3 pr-6 w-36">Attribute</th>
                {instances.map((inst) => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {rows.map((row) => {
                ...
                <tr key={row.label} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  <td className="py-2.5 pr-6 text-sm text-gray-500 dark:text-gray-400 font-medium">{row.label}</td>
                  {instances.map((inst) => {
                    ...
                    return (
                      <td key={inst.name} className={`py-2.5 px-4 ${cellColor}`}>
                        <span className={`font-mono text-sm ${isUnavailable ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'}`}>
                          {val}
                        </span>
                      </td>
                    )
